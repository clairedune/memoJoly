...
The algorithm to process Kinect images is presented in this section. \CD{The method aims at fitting a 3D skeleton on the partial kinect data. The 2 legs will be described as two rigid bodies linked with a ball joint (which represent an approximation of the ankle joint).}{The goal of the method is to segment  Kinect points cloud (see Fig.~\ref{}) in two part (left and right) which are composed by}:
\begin{itemize}
...
The main idea of feet and legs segmentation is based on region growing. It is inspired by \TODO{\cite{aaa}}. \CD{The segmentation is perform directly in a 2D image that corresponds to the depth map (using directly the depth map instead of the 3D point cloud significantly speed up the process)}{}. From \CD{an initial}{a current} set of points \CD{belonging to}{of} a member, we look for potential candidates in the neighbouring\CD{. Then, according to some distance \textit{criteria} we decide to keep or discard them. \\}{ and then validate it or not. } The following paragraphs are dedicated to the presentation of the process. The global algorithm is presented in paragraph~\ref{subsub:generalAlgo}. Then application for legs and feet segmentation is presented in paragraph~\ref{subsub:legSegmentation} and~\ref{subsub:footSegmentation}.
...
\begin{enumerate}
	...
	\begin{equation}
		...
		\label{eq:defV}
	\end{equation}
	where $s$ is a threshold \CD{arbitrarily defined, \textit{e.g.} 8-10 pixel in the depth map }{to define (8-10 pixels)}. Note that in \eqref{eq:defV} $\mathbf{m}$ and $\mathbf{v}$ are not 3D points but pixels in Kinect \CD{depth map}{image}.
...
\begin{enumerate}
	\item Let $\mathcal{B}=(\mathbf{u},\mathbf{v},\mathbf{w})$ be an orthonormal base with $\mathbf{w}$ in the same direction \CD{as}{than} the cylinder axis and  $\mathbf{C}$ a point belonging to this axis. \CD{We can define the rotation matrix $\mathbf{R} = \begin{bmatrix}\mathbf{u} & \mathbf{v} & \mathbf{w}\end{bmatrix}$ associated to the cylinder orientation.}{The rotation matrix $\mathbf{R}$  $(\mathbf{x},\mathbf{y},\mathbf{z})$ initiale à $\mathcal{B}$:
	\begin{equation}
		\mathbf{R} = 
		\begin{bmatrix}
...
Fig.~\ref{fig:footError} show the results of error in orientation of the feet. It can be seen that the elevation angle (computed with respect to the ground) is pretty well estimated (less than 5deg of error in most cases). This is an important result that show that the system may be able to pretty well estimate if a foot is on the ground or not. 
