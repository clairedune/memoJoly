...
\begin{enumerate}
	\item Let $\mathcal{B}=(\mathbf{u},\mathbf{v},\mathbf{w})$ be an orthonormal base with $\mathbf{w}$ in the same direction \CD{as}{than} the cylinder axis and  $\mathbf{C}$ a point belonging to this axis. The rotation matrix $\mathbf{R}$  $(\mathbf{x},\mathbf{y},\mathbf{z})$ initiale à $\mathcal{B}$:
	\begin{equation}
		\mathbf{R} = 
		\begin{bmatrix}
